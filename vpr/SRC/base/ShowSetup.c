...
static void ShowRouterOpts(INP struct s_router_opts RouterOpts) {
	...
	if (DETAILED == RouterOpts.route_type) {
		...
		case NO_TIMING:
			...
			break;
		case READ_ROUTE:  //added QT
			vpr_printf(TIO_MESSAGE_INFO, "READ_ROUTE\n");
			break;
		default:
			vpr_printf(TIO_MESSAGE_INFO, "<Unknown>\n");
			exit(1);
	...
	{
		vpr_printf(TIO_MESSAGE_INFO, "RouterOpts.inc_type:             ");
		switch(RouterOpts.inc_type)
		{
		case INC_OVERLAY: vpr_printf(TIO_MESSAGE_INFO, "INC_OVERLAY\n"); break;
		default:
		    vpr_printf(TIO_MESSAGE_ERROR, "<Unknown>\n");
		    exit(1);
		}

    	vpr_printf(TIO_MESSAGE_INFO, "RouterOpts.inc_router_algorithm:  ");
    	...
    	vpr_printf(TIO_MESSAGE_INFO, "RouterOpts.inc_le_symmetry:   %s\n", RouterOpts.inc_le_symmetry?"TRUE":"FALSE");
    	vpr_printf(TIO_MESSAGE_INFO, "RouterOpts.inc_best_effort:   %s\n", RouterOpts.inc_best_effort?"TRUE":"FALSE");
    	vpr_printf(TIO_MESSAGE_INFO, "RouterOpts.inc_connectivity:  %d\n", RouterOpts.inc_connectivity);
    	vpr_printf(TIO_MESSAGE_INFO, "RouterOpts.inc_match_file:    %s\n", RouterOpts.inc_match_file);
	}
    vpr_printf(TIO_MESSAGE_INFO, "\n");
	/* END Eddie */
}

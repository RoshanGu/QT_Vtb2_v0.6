...
#include "util.h"
#include "vpr_types.h"    //contains the struct s_file_name_opts
#include "vpr_utils.h"
#include "globals.h"
...
//modified for QT with new struct
...
void place_and_route(enum e_operation operation,
		struct s_placer_opts placer_opts, struct s_file_name_opts *FileNameOpts,
		struct s_annealing_sched annealing_sched,
		struct s_router_opts router_opts,
		struct s_det_routing_arch det_routing_arch, t_segment_inf * segment_inf,
		t_timing_inf timing_inf, t_chan_width_dist chan_width_dist,
		struct s_model *models,
		t_direct_inf *directs, int num_directs, t_arch *Arch) {
	...
	int num_tb;        //added as part of Qt
	t_trace_buffer *tb;
	...
	if (NO_FIXED_CHANNEL_WIDTH == width_fac) {
		g_solution_inf.channel_width = binary_search_place_and_route(placer_opts, FileNameOpts, router_opts.full_stats,
		...
		success = try_route(width_fac, router_opts, det_routing_arch,
				...
				clb_opins_used_locally, &Fc_clipped, operation, directs, FileNameOpts, num_directs,
				FileNameOpts->ArchFile); //changed for QT
    	...
    	if (router_opts.inc_type != INC_NONE)
    	{
			...
			if (success)
			{
				...
				/*added QT - as per VTR 7.0 -  *
				which also have re-organised the corresponding structure in vpr_types.h */
				
				do_timing_analysis(slacks, FALSE, FALSE, TRUE); 
				T_crit = get_critical_path_delay();
		    	
				//T_crit = load_net_slack(slacks->slack, 0);
		...
		if (success == FALSE) {
			...
			//print_route(route_file);   //added QT
#ifdef PRINT_TIMING
		    print_net_delay(net_delay, "before.net_delay", clb_net, num_nets);
		    print_net_slack("before.net_slack", slacks->slack);
	...
	while (final == -1) {
		...
		if (placer_opts.place_freq == PLACE_ALWAYS) {
			...
			try_place(placer_opts, annealing_sched, chan_width_dist,
					...
					directs, num_directs, FileNameOpts->ArchFile);
		}
		free_rr_node_route_structs(); //added QT, changed try_route below
		success = try_route(current, router_opts, det_routing_arch, segment_inf,
				timing_inf, net_delay, slacks, chan_width_dist,
				clb_opins_used_locally, &Fc_clipped, RUN_FLOW, directs, FileNameOpts, num_directs, FileNameOpts->ArchFile);
		attempt_count++;
		fflush(stdout);
#if 1
...
//#endif
	...
	if (verify_binary_search) {
		...
		while (prev2_success || prev_success) {
			...
			success = try_route(current, router_opts, det_routing_arch,
					...
					chan_width_dist, clb_opins_used_locally, &Fc_clipped, RUN_FLOW, directs, FileNameOpts, NULL, FileNameOpts->ArchFile);
	...
	build_rr_graph(graph_type, num_types, type_descriptors, nx, ny, grid,
			...
			directs, num_directs, FALSE, FileNameOpts->ArchFile,
			&warnings);

	restore_routing(best_routing, clb_opins_used_locally,
			saved_clb_opins_used_locally);
	recompute_occupancy_from_scratch_no_check(clb_opins_used_locally); //added QT

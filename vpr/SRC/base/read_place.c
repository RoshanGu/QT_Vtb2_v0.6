...
void read_place(INP const char *place_file, INP const char *arch_file,
	...
	if (0 != strcmp(tokens[2], arch_file)) {
		vpr_printf(TIO_MESSAGE_WARNING, "'%s' - Architecture file that generated placement (%s) does not match current architecture file (%s).\n", 
	...
	while (tokens) {
		...
		for (i = 0; i < L_num_blocks; ++i) {
			/* FIXME: HACK because all trace-buffers are called "unconn"! -- added QT
